...
- 0.10.3 の Platform, Draft, Public Contract and CI Hardening では、canonical core decoder、Windows のみ case-insensitive な path dedupe、PHP / Python model draft の identifier / literal safety と `syntaxValidation`、stable read-only 17 tool の MCP `outputSchema`、connection exact / unknown total を示す nullable `totalConnectionCount`、CLI 直実行の既定 masking と `--unsafe-raw-output` の警告境界を確認する。PR / main は Ubuntu / Windows CI、tag publish は publish 前の `published:check` を必須にする。
- 0.10.4 の MCP Resource Gateway Pilot では、`a5sql://configured-file/summary` と `.a5er` 専用の `a5sql://configured-file/schema-summary` を `resources/list` / `resources/read` で確認する。Resource は summary-only、path-free、秘密情報マスク済みとし、A5:SQL 由来 payload は `contentIsUntrusted: true` / `untrustedPayloadFields` で未信頼として扱う。明示 `roots` / `A5SQL_MCP_ROOTS` が必要な asset 操作は tool-only とし、root/path を Resource URI に含めない。`--tool-profile` は Resource filtering に使わず、DB 接続、SQL 実行、ファイル書き込みを追加しない。
