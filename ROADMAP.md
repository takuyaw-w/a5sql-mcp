...
- tool 数が 20 を超えており、LLM が `tools/list` だけから正しい tool を選ぶ負荷が上がっている。1.0.0 前の 0.10.x で、互換性を保った tool surface の profile 化を検討する余地がある。
- tool description は LLM 向け API の主要な選択材料だが、説明の粒度、使う場面、返す内容、read-only / draft 境界を横断的に監査する余地がある。
...
### [ ] 0.10.2: Structured Errors and Safe Observability
...
- MCP startup、`tools/list`、代表 `callTool` の published-style validation が通る。

### [ ] 0.10.3: MCP Resource Gateway Pilot
...
### [ ] 0.10.4: 0.10.x Dry Run / Draft Graduation Decision

目的:

- 0.10.x の architecture / profile / observability / resource gateway 検討を締め、生成補助 tool の `experimental draft` 卒業を 1.0.0 前に行うか判断する。
...
- 卒業に足りない contract gap がある場合は、0.10.4 に混ぜず 0.11.0 以降に切り出す。
...
5. 0.10.0 で architecture pattern と tool description の監査を行う。
6. 0.10.1 で optional profile による tool 数削減を設計する。
7. 0.10.2 で structured error と安全な observability を整える。
8. 0.10.3 で MCP Resource Gateway として expose できる summary を検証する。
9. 0.10.4 で 0.10.x の dry run と draft graduation の実施判断を行う。
10. 0.11.0 で `experimental draft` を外すための graduation criteria を固定する。
11. 0.12.0 で卒業対象の draft helper contract と validation を固める。
12. 0.13.0 で `experimental` marker を外し、1.0.0 final RC を行う。
