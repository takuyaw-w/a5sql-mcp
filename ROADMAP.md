...
- 1.0.0 の直前候補として、機能追加を止めて release readiness を確認する。

主な作業:

- `rtk pnpm release:check` と `rtk pnpm published:check` を実行する。
- tarball install 後の MCP startup、`tools/list`、代表 `callTool`、adversarial fixture の代表ケースを確認する。
- README、AGENTS.md、skill、package metadata、version、tag preflight を確認する。
- 1.0.0 に含めない DB 接続、SQL 実行、書き込み、資格情報復号、Web UI、daemon が docs で明確か確認する。

完了条件:

- 1.0.0 として tag/publish する前に必要な検証が再現可能である。
- 新機能追加ではなく、修正が必要な regression だけを扱う状態になっている。
...
1. prompt injection 防衛の baseline を作る。
2. trusted metadata と untrusted payload を分離する。
3. 秘密情報マスクと path privacy を再監査する。
4. `roots` / `A5SQL_MCP_ROOTS` の境界を hardening する。
5. hostile fixture を使った MCP E2E を増やす。

次点:

1. parser fixture の拡充
2. README の client / agent safety docs 整理
3. skill / AGENTS / README の tool 一覧同期
4. API freeze rehearsal
5. `tool-outputs.ts` 周辺の段階的分割
